...
class PhysicsEngine {
public:
    PhysicsEngine(const int WIDTH, const int HEIGHT) : WIDTH(1440), HEIGHT(900) {};
...
private:
    void initWindow() {
        ...
        window = glfwCreateWindow(this->WIDTH, this->HEIGHT, "thing", nullptr, nullptr);
    }
    ...
    static void checkExtensions(const std::vector<const char *> &supportedExtensions,
        ...
        for (VkExtensionProperties extension : extensions) {
            ...
            for (const char *supportedExtension : supportedExtensions) {
                if (!std::strcmp(supportedExtension, extension.extensionName)) {
...
int main() {
    PhysicsEngine application(1440, 900);
